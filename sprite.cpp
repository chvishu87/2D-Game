...
Sprite::Sprite(const Sprite& s) :
  ...
  //frame(s.getFrame())
	frame(s.getFrame())
  { //frame = s.getFrame();
		//std::cout<<"I am being created  "<<name<<std::endl;
		//std::cout<<frame->getWidth()<<'\t'<<frame->getHeight()<<std::endl;
		//std::cout<<s.getFrame()->getWidth()<<'\t'<<s.getFrame()->getHeight()<<std::endl;
	}
...
bool Sprite::inView() {
	if(X() >= (Viewport::getInstance()).X() &&
		 Y() >= (Viewport::getInstance()).Y() &&
		 X() <= ((Viewport::getInstance()).X()) + Gamedata::getInstance()->getXmlInt("viewWidth") &&
		 Y() <= ((Viewport::getInstance()).Y()) + Gamedata::getInstance()->getXmlInt("viewHeight"))
		return true;
	else
		return false;
}
...
/*unsigned Sprite::getPixel(Uint32 i, Uint32 j) const {
	...
	Uint32 *pixel = static_cast<Uint32>(frame->getSurface()->pixels);
	return pixels[ (y * frame->getWidth() + x) ];
}*/
